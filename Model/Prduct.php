...
class Prduct extends Database
{
    ...
    public function notify()
    {
        ...
        $sql = "INSERT INTO notification (subject, message, date_in)
    ...
    {
        $sql = "select * from item WHERE ITEM_STATUS = 'ACTIVE' >= 1";
        $stmt = $this->getConnect()->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    ...
    public function filterItem()
    {
        ...
        t.CATEGORY_ID = c.CATEGORY_ID WHERE t.CATEGORY_ID = {$this->id} AND ITEM_STATUS = 'ACTIVE' GROUP BY ITEM_NAME";
        $stmt = $this->getConnect()->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    ...
    {
        $sql = "SELECT * FROM " . $this->table . " inner join category ON item.CATEGORY_ID = category.CATEGORY_ID join supplier on item.SUPPLIER_ID = supplier.SUPPLIER_ID WHERE item.ITEM_CODE = {$this->id} AND item.ITEM_STATUS = 'ACTIVE' GROUP BY supplier.SUPPLIER_COMPANY";
        $stmt = $this->getConnect()->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    ...
    {
        $sql = "SELECT * FROM {$this->table} where SUPPLIER_ID = {$this->supp_id} AND ITEM_STATUS = 'ACTIVE'";
        $stmt = $this->getConnect()->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
